/* proc.h - isbadpid */
/* PA1
 * 	add two variable to pentry structure (goodness, counter).
 * */
#ifndef _PROC_H_
#define _PROC_H_
...
struct	pentry	{
	...
	int	pwaitret;
	/* PA1 */
	int	goodness;		/* update every resched(), use to schedule next process */
	int	counter;		/* record remaining time quantum in previous epoch	*/
};
