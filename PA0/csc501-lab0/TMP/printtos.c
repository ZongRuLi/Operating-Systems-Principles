...
void printtos()
{
	 ...
	 * by increasing stack pointer by 0x04 (ebp + 1). Now we point to return address.
	 *
	 * Moreover, to get ESP before program push into return address, we need to increase stack pointer again.
	 * which is (ebp+2).
	 * */
	...
	kprintf("\nBefore[0x%08x]: 0x%08x",prev, *prev);
	kprintf("\nAfter [0x%08x]: 0x%08x",curr, *curr);

	//Printing upto four stack locations below the top of the stack 
	int i;
	for(i=0; i<4; i++) {
		kprintf("\n\telement[0x%08x] : 0x%08x", (curr-i),*(curr-i));
	}
}
